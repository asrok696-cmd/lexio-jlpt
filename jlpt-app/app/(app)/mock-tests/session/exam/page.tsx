// app/mock-tests/session/exam/page.tsx
"use client";
...
export default function MockExamPage() {
  ...
  const level = ((sp.get("level") ?? "N5").toUpperCase() as JLPTLevel) ?? "N5";
  const slot = Number(sp.get("slot") ?? "1") || 1;
  const startISOFromQS = parseStartISO(sp.get("start"));
  ...
  useEffect(() => {
    ...
    // set once immediately (in case hydration delay)
    setElapsedSec(() => {
      const startMs = new Date(startedAtISO).getTime();
